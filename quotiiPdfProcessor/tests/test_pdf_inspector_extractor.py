...
class TestPdfInspectorExtractor(unittest.TestCase):
    ...
    @patch('services.pdf.pdfInspectorExtractor.pdf_inspector')
    def test_writes_one_markdown_file_per_page(self, mock_pi):
        ...
        self.assertIn('page one markdown', open(os.path.join(out_dir, 'page-1.md')).read())
    ...
    def test_mixed_uses_per_page_needs_ocr_flags(self, mock_pi):
        ...
        self.assertEqual(
            open(os.path.join(result, 'page-2.md')).read(),
            'digital text',
        )
